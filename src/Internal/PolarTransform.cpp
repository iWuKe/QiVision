...
template<typename T>
void WarpCartesianToPolar(const T* src, int32_t srcWidth, int32_t srcHeight,
    ...
    for (int32_t row = 0; row < dstHeight; ++row) {
        ...
        for (int32_t col = 0; col < dstWidth; ++col) {
            ...
            // Sample and store
            double value = SamplePixel(src, srcWidth, srcHeight, srcX, srcY,
            ...
            dst[row * dstWidth + col] = static_cast<T>(value);
        }
    }
}
...
template<typename T>
void WarpPolarToCartesian(const T* src, int32_t srcWidth, int32_t srcHeight,
                          T* dst, int32_t dstWidth, int32_t dstHeight,
                          const Point2d& center, double maxRadius,
                          PolarMode mode,
                          InterpolationMethod method,
    ...
    for (int32_t y = 0; y < dstHeight; ++y) {
        for (int32_t x = 0; x < dstWidth; ++x) {
            ...
            double srcX = theta / TWO_PI * static_cast<double>(srcWidth);
            double srcY = rho / maxRadius * static_cast<double>(srcHeight);
    ...
    for (int32_t c = 0; c < channels; ++c) {
        switch (src.Type()) {
            case PixelType::UInt8: {
                ...
                const uint8_t* srcData = srcBase + c * srcWidth * srcHeight;
                uint8_t* dstData = dstBase + c * dstWidth * dstHeight;

                if (!inverse) {
                    WarpCartesianToPolar(srcData, srcWidth, srcHeight,
                                         dstData, dstWidth, dstHeight,
                                         center, maxRadius, mode,
                                         method, borderMode, borderValue);
                } else {
                    WarpPolarToCartesian(srcData, srcWidth, srcHeight,
                                         dstData, dstWidth, dstHeight,
                                         center, maxRadius, mode,
                                         method, borderMode, borderValue);
                }
            ...
            case PixelType::UInt16: {
                ...
                const uint16_t* srcData = srcBase + c * srcWidth * srcHeight;
            ...
            case PixelType::Int16: {
                ...
                const int16_t* srcData = srcBase + c * srcWidth * srcHeight;
            ...
            case PixelType::Float32: {
                ...
                const float* srcData = srcBase + c * srcWidth * srcHeight;
